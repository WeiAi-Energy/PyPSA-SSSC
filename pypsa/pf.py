...
def find_cycles(
    sub_network: SubNetwork,
    weight: str = "x_pu",
    config: cycle_basis.CycleBasisConfig | None = None,
) -> None:
    """
    Find all cycles in the sub_network and record them in sub_network.C.

    The whole construction lives in :mod:`pypsa.cycle_basis`; see
    :func:`pypsa.cycle_basis.build_cycle_matrix` for what it does and
    :class:`pypsa.cycle_basis.CycleBasisConfig` for the one thing a caller
    can change about it.
    """
    sub_network.C = cycle_basis.build_cycle_matrix(sub_network, weight, config)
